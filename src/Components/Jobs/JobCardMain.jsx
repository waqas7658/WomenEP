...
const JobCardMain = ({ job, }) => {
    ...
    const handleApplyJob = async (jobId, user) => {
        ...
        try {
            if (user) {
                const response = await axios.post(`${BASEURL}/api/career/career-development-apply`, reqPacket)
    ...
    return (
        <div className="max-w-sm rounded-lg overflow-hidden shadow-lg m-4 bg-white">
            ...
            <div className="px-6 py-4">
                ...
                <div className=' my-2'>
                    <button onClick={() => handleApplyJob(job?._id, user)} className="py-2 px-4 bg-pink-700 hover:bg-pink-500 text-white rounded"
                    >Apply</button>
                </div>
            </div>
        </div>
